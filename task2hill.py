...
def DrawSurface(fig, varxrange, varyrange, function):
    ...
    #ax.plot_surface(xx, yy, z, cmap='RdBu').pcolor(Z, edgecolors='k', linewidths=1)
...
def GradAscent(StartPt,NumSteps,LRate):
    ...
    for i in range(NumSteps):
        ...
        NewPt[0] = StartPt[0] + LRate * gradient
        NewPt[1] = StartPt[1] + LRate * gradient
        StartPt[0]=NewPt[0]
        StartPt[1]=NewPt[1]
...
def HillClimb(StartPt,NumSteps,MaxMutate):
    ...
    for i in range(NumSteps):
        ...
        plt.scatter(StartPt[0],StartPt[1],color='k',s=10)
        #use of pcolor 
        #plt.scatter(StartPt[0],StartPt[1],color='k',s=10).pcolor(Z, edgecolors='k', linewidths=1)
        # Use a markersize that you can see well enough (e.g., * in size 10)
        plt.xlabel("X")
        plt.xlabel("Y")
        #plt.plot(StartPt[0],height, ls="", marker="o", label="points")
        #plt.plot(StartPt[0], height, 'g*')
        ...
        height = ComplexLandscape(NewPt[0], NewPt[1])        
...
StartPt=[]
for x in range (0,2):
    StartPt.append(random.randint(-10, 10))
#StartPt = [-3,7]
# Find maximum (un/comment relevant line)
#value = GradAscent(StartPt,NumSteps,LRate)
value=HillClimb(StartPt,NumSteps,MaxMutate)
if value[0] == 1:
    print("Optimal maximum reached")
print('No. of required iteration', value[1])
